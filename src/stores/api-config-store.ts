...
const PROVIDER_INFO: Partial<Record<ProviderId, { name: string; services: ServiceType[] }>> = {
  custom: { name: 'Custom', services: ['chat', 'image', 'video', 'vision'] },
};
...
// Pre-fill Custom provider for new users (no API Key, just the provider entry)
const defaultProviderTemplate = DEFAULT_PROVIDERS.find(p => p.platform === 'custom');

const initialState: APIConfigState = {
  providers: defaultProviderTemplate
    ? [{ id: generateId(), ...defaultProviderTemplate, apiKey: '' }]
    : [],
