...
export const useDirectorStore = create<DirectorStore>()(
  ...
  // Screenplay management
  setScreenplay: (screenplay: AIScreenplay | null) => {
  ...
  setScreenplayStatus: (status: ScreenplayStatus) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    set({
      projects: {
  ...
  setScreenplayError: (error: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
  ...
  // Scene editing
  updateScene: (sceneId: number, updates: Partial<AIScene>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    if (!project?.screenplay) return;
    
    const updatedScenes = project.screenplay.scenes.map((scene: AIScene) => 
  ...
  deleteScene: (sceneId: number) => {
    ...
    const remainingScenes = project.screenplay.scenes.filter((scene: AIScene) => scene.sceneId !== sceneId);
    const renumberedScenes = remainingScenes.map((scene: AIScene, index: number) => ({
      ...scene,
      sceneId: index + 1,
    }));
    
    const newProgressMap = new Map<number, SceneProgress>();
    remainingScenes.forEach((scene: AIScene, index: number) => {
  ...
  // Scene progress
  updateSceneProgress: (sceneId: number, partialProgress: Partial<SceneProgress>) => {
  ...
  setSceneProgress: (sceneId: number, progress: SceneProgress) => {
    set((state: DirectorStore) => {
      const newMap = new Map(state.sceneProgress);
  ...
  // Config
  updateConfig: (partialConfig: Partial<GenerationConfig>) => set((state: DirectorStore) => ({
    config: { ...state.config, ...partialConfig }
  })),

  // UI
  setExpanded: (expanded: boolean) => set({ isExpanded: expanded }),
  setSelectedScene: (sceneId: number | null) => set({ selectedSceneId: sceneId }),

  // Storyboard actions (new workflow) - Project-aware
  setStoryboardImage: (imageUrl: string | null, mediaId?: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    set({
      projects: {
  ...
  setStoryboardStatus: (status: StoryboardStatus) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    set({
      projects: {
  ...
  setProjectFolderId: (folderId: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    set({
      projects: {
  ...
  setStoryboardError: (error: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const currentProject = projects[activeProjectId];
    set({
  ...
  // 更新首帧提示词（静态画面描述）
  updateSplitSceneImagePrompt: (sceneId: number, prompt: string, promptZh?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
      scene.id === sceneId ? { 
        ...scene, 
        imagePrompt: prompt,
  ...
  // 更新视频提示词（动作过程描述）
  updateSplitSceneVideoPrompt: (sceneId: number, prompt: string, promptZh?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 更新尾帧提示词（静态画面描述）
  updateSplitSceneEndFramePrompt: (sceneId: number, prompt: string, promptZh?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 设置是否需要尾帧
  updateSplitSceneNeedsEndFrame: (sceneId: number, needsEndFrame: boolean) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 兼容旧 API：更新视频提示词（实际上更新 videoPrompt）
  updateSplitScenePrompt: (sceneId: number, prompt: string, promptZh?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneImage: (sceneId: number, imageDataUrl: string, width?: number, height?: number, httpUrl?: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneImageStatus: (sceneId: number, updates: Partial<Pick<SplitScene, 'imageStatus' | 'imageProgress' | 'imageError'>>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneVideo: (sceneId: number, updates: Partial<Pick<SplitScene, 'videoStatus' | 'videoProgress' | 'videoUrl' | 'videoError' | 'videoMediaId'>>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneEndFrame: (sceneId: number, imageUrl: string | null, source?: 'upload' | 'ai-generated' | 'next-scene' | 'video-extracted', httpUrl?: string | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 更新尾帧生成状态
  updateSplitSceneEndFrameStatus: (sceneId: number, updates: Partial<Pick<SplitScene, 'endFrameStatus' | 'endFrameProgress' | 'endFrameError'>>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneCharacters: (sceneId: number, characterIds: string[]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneEmotions: (sceneId: number, emotionTags: EmotionTag[]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneShotSize: (sceneId: number, shotSize: ShotSizeType | null) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneDuration: (sceneId: number, duration: DurationType) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneAmbientSound: (sceneId: number, ambientSound: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  updateSplitSceneSoundEffects: (sceneId: number, soundEffects: SoundEffectTag[]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 场景库关联更新方法（首帧）
  updateSplitSceneReference: (sceneId: number, sceneLibraryId?: string, viewpointId?: string, referenceImage?: string, subViewId?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 场景库关联更新方法（尾帧）
  updateSplitSceneEndFrameReference: (sceneId: number, sceneLibraryId?: string, viewpointId?: string, referenceImage?: string, subViewId?: string) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 通用字段更新方法（用于双击编辑）
  updateSplitSceneField: (sceneId: number, field: keyof SplitScene, value: any) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) =>
  ...
  // 视角切换历史记录更新方法
  addAngleSwitchHistory: (sceneId: number, type: 'start' | 'end', historyItem: { imageUrl: string; angleLabel: string; timestamp: number }) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const updated = project.splitScenes.map((scene: SplitScene) => {
  ...
  deleteSplitScene: (sceneId: number) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const remaining = project.splitScenes.filter((s: SplitScene) => s.id !== sceneId);
    const renumbered = remaining.map((s: SplitScene, idx: number) => ({ ...s, id: idx }));
    set({
      projects: {
        ...projects,
  ...
  setStoryboardConfig: (partialConfig: Partial<DirectorProjectData['storyboardConfig']>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    set({
  ...
  addScenesFromScript: (scenes: Parameters<DirectorActions['addScenesFromScript']>[0]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    const splitScenes = project?.splitScenes || [];
    const startId = splitScenes.length > 0 ? Math.max(...splitScenes.map((s: SplitScene) => s.id)) + 1 : 1;
  ...
  // Workflow actions
  startScreenplayGeneration: (prompt: string, images?: File[]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId] || defaultProjectData();
    set({
      projects: {
        ...projects,
        [activeProjectId]: {
  ...
  startImageGeneration: () => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) {
      console.error('[DirectorStore] No active project to generate images');
      return;
    }
    const project = projects[activeProjectId];
  ...
  startVideoGeneration: () => {
    const { activeProjectId, projects, sceneProgress } = get();
    if (!activeProjectId) {
      console.error('[DirectorStore] No active project to generate videos');
      return;
    }
    const project = projects[activeProjectId];
    if (!project?.screenplay) {
  ...
  retrySceneImage: (sceneId: number) => {
  ...
  // Worker callbacks
  onScreenplayGenerated: (screenplay: AIScreenplay) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId] || defaultProjectData();
    set({
  ...
  onSceneProgressUpdate: (sceneId: number, progress: SceneProgress) => {
    get().setSceneProgress(sceneId, progress);
  },

  // Called when a scene's image is generated
  onSceneImageCompleted: (sceneId: number, imageUrl: string) => {
    ...
    const { activeProjectId, projects } = get();
    const screenplay = activeProjectId ? projects[activeProjectId]?.screenplay : null;
    if (screenplay) {
    ...
    const { sceneProgress } = get();
    if (screenplay) {
      const allImagesDone = screenplay.scenes.every((scene: AIScene) => {
  ...
  onSceneCompleted: (sceneId: number, mediaId: string) => {
    get().updateSceneProgress(sceneId, {
      status: 'completed',
      stage: 'done',
      progress: 100,
    ...
    const { sceneProgress, activeProjectId, projects } = get();
    const screenplay = activeProjectId ? projects[activeProjectId]?.screenplay : null;
    if (screenplay) {
      const allDone = screenplay.scenes.every((scene: AIScene) => {
        const progress = sceneProgress.get(scene.sceneId);
  ...
  onSceneFailed: (sceneId: number, error: string) => {
    get().updateSceneProgress(sceneId, {
      status: 'failed',
      error,
    });
  ...
  setTrailerDuration: (duration: TrailerDuration) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    set({
  ...
  setTrailerScenes: (scenes: SplitScene[]) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    set({
  ...
  setTrailerConfig: (config: Partial<TrailerConfig>) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    set({
  ...
  setCinematographyProfileId: (profileId: string | undefined) => {
    const { activeProjectId, projects } = get();
    if (!activeProjectId) return;
    const project = projects[activeProjectId];
    set({
...
export const useOverallProgress = (): number => {
  return useDirectorStore((state) => {
    const { activeProjectId, projects, sceneProgress } = state;
    const screenplay = activeProjectId ? projects[activeProjectId]?.screenplay : null;
